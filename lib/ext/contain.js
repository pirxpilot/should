...
import { forEach, isEmpty, isIterable, iterator, some } from 'should-type-adaptors';
...
export default function (should, Assertion) {
  ...
  Assertion.add('containEql', function (other) {
    ...
    if (typeof obj === 'string') {
      ...
      this.assert(some(obj, v => eql(v, other).length === 0));
    } else {
      forEach(
        other,
        (value, key) => {
  ...
  Assertion.add('containDeepOrdered', function (other) {
    ...
    if (typeof obj === 'string') {
      ...
      const otherIterator = iterator(other);

      let nextObj = objIterator.next();
      let nextOther = otherIterator.next();
